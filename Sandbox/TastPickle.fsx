...
#load "./TastPickle.Types.fsx"
open TastPickle.Types
...
#else
    let msg = sprintf "%s: %s" fileName str
#endif
...
let makeNodeInTable mkEmpty lnk isLinked nm length = 
    {
        LinkNode = lnk;
        IsLinked = isLinked;
        Name = nm;
        Nodes = Array.init length (fun _i -> mkEmpty() )
    }
...
// CCU References 
// A huge number of these occur in pickled F# data, so make them unique 
let encode_ccuref ccuTab (x:CcuThunk) = encode_uniq ccuTab (q"%A.AssemblyName" x)
...
// A huge number of these occur in pickled F# data, so make them unique 
let decode_pubpath st stringTab a = q"PubPath(%A)" (Array.map (lookup_string st stringTab) a)
...
// A huge number of these occur in pickled F# data, so make them unique. 
let decode_simpletyp st _ccuTab _stringTab nlerefTab a = q"TType_app(%A, %A)" (q"ERefNonLocal(%A)" (lookup_nleref st nlerefTab a)) []
...
let unpickleObjWithDanglingCcus file ilscope (iILModule:ILModuleDef) u (phase2bytes:byte[]) =
    let st2 = {
        is = ByteStream.FromBytes(phase2bytes, 0); 
        iilscope = ilscope;
        iccus = new_itbl "iccus (fake)" [| |]; 
        itycons = makeNodeInTable Tycon.NewUnlinked Tycon.Link Tycon.IsLinked "itycons" 0; 
        itypars = makeNodeInTable Typar.NewUnlinked Typar.Link Typar.IsLinked "itypars" 0; 
        ivals = makeNodeInTable Val.NewUnlinked Val.Link Val.IsLinked "ivals" 0;
    ...
    let data = 
        let st1 = 
           { is = ByteStream.FromBytes (phase1bytes, 0); 
             iccus = ccuTab; 
             iilscope = ilscope;
             itycons = makeNodeInTable Tycon.NewUnlinked Tycon.Link Tycon.IsLinked "itycons" ntycons; 
             itypars = makeNodeInTable Typar.NewUnlinked Typar.Link Typar.IsLinked "itypars" ntypars; 
             ivals = makeNodeInTable Val.NewUnlinked Val.Link Val.IsLinked "ivals" nvals;
             istrings = stringTab;
    ...
    q"{ RawData = %A; FixupThunks = %A }" data (Array.toList ccuTab.itbl_rows)
...
let u_ILPublicKey st = 
    let tag = u_byte st
    match tag with
    | 0 -> u_bytes st |> q"PublicKey(%A)"
    | 1 -> u_bytes st |> q"PublicKeyToken(%A)"
...
let u_ILModuleRef st = 
    ...
    q"{ name = %A; hasMetadata = %A; hash = %A; }" a b c
...
let u_ILAssemblyRef st =
    ...
    | 0 -> 
        ...
        q"ILAssemblyRef({
            assemRefName = %A;
              assemRefHash = %A;
              assemRefPublicKeyInfo = %A;
              assemRefRetargetable = %A;
              assemRefVersion = %A;
              assemRefLocale = %A; })" a b c d e f
...
let u_ILScopeRef st = 
    let res = 
        let tag = u_byte st
        match tag with
        | 0 -> q"ILScopeRef.Local"
        | 1 -> u_ILModuleRef st |> q"ILScopeRef.Module(%A)"
        | 2 -> u_ILAssemblyRef st |> q"ILScopeRef.Assembly(%A)"
        | _ -> ufailwith st "u_ILScopeRef"  
    let res = q"rescopeILScopeRef %A %A" st.iilscope res 
    res
...
let u_ILBasicCallConv st = 
    match u_byte st with 
    | 0 -> q"ILArgConvention.Default"
    | 1 -> q"ILArgConvention.CDecl"
    | 2 -> q"ILArgConvention.StdCall"
    | 3 -> q"ILArgConvention.ThisCall"
    | 4 -> q"ILArgConvention.FastCall"
    | 5 -> q"ILArgConvention.VarArg"
    | _ -> ufailwith st "u_ILBasicCallConv"

let u_ILHasThis st = 
    match u_byte st with 
    | 0 -> q"ILThisConvention.Instance"
    | 1 -> q"ILThisConvention.InstanceExplicit"
    | 2 -> q"ILThisConvention.Static"
    | _ -> ufailwith st "u_ILHasThis"

let u_ILCallConv st = let a,b = u_tup2 u_ILHasThis u_ILBasicCallConv st in q"Callconv(%A, %A)" a b
let u_ILTypeRef st = let a,b,c = u_tup3 u_ILScopeRef u_strings u_string st in ILTypeRef.Create(a, b, c) 
let u_ILArrayShape = u_wrap (q"ILArrayShape(%A)") (u_list (u_tup2 (u_option u_int32) (u_option u_int32)))

let pipe2 p1 p2 st f = f (p1 st) (p2 st)

let rec u_ILType st = 
    let tag = u_byte st
    match tag with
    | 0 -> q"ILType.Void"
    | 1 -> pipe2 u_ILArrayShape u_ILType  st <| q"ILType.Array(%A, %A)"
    | 2 -> u_ILTypeSpec st |> q"ILType.Value(%A)"
